...
# Case 1: Mgus to MM ------
# read data --------------------------------------
mgus_to_mm <- read_sas("mgus_to_symmm_v5.sas7bdat")
nrow(mgus_to_mm)
...
for (i in 1:4){
  ...
  ggsave(paste0("250415_ScoringHist_", labels[i], ".pdf"),height=7,width=10,dpi=300)
...
################
dev.new()
pdf('250415_ggsurvplot_1yr.pdf',height=10,width=10)
...
# only result table
dev.new()
pdf('250415_ggsurvplot_1yr_onlytable.pdf',height=5,width=5)
...
##################
dev.new()
pdf('250415_Adj_ggsurvplot_1yr.pdf',height=10,width=13)
...
pdf('250415_Adj_ggsurvplot_1yr_onlytable.pdf',height=5,width=5)
...
dev.new()
pdf("250415_smdplot.pdf",height=7,width=12)
tab_smd_res %>%
...
pdf("250415_mgus_ci.pdf", height=5, width=6.5)
print(p)
dev.off()
...
pdf("250415_mgus_ci_onlytable.pdf", height=5, width=6.5)
print(p)
dev.off()
...
# 6mths landmark ------
# source("./R_spj/MGUS_MM_function.R")
landmark_analysis <- function(year_num){
  
  scoring_MM_landmark <- scoring_MM[scoring_MM$death_year >= year_num,]
  my_weights = get_sw(group1 ~ ageg4 + SEX_TP_CD + CCI_score + Doublet + Low_intensity_triplet + High_intensity_triplet, data = scoring_MM_landmark)
  
  scoring_MM_landmark$group = relevel(scoring_MM_landmark$group, ref=3)
  fit_scoring_MM_landmark <- coxph(Surv(death_year, event) ~ group, data=scoring_MM_landmark, weights= my_weights$weight, robust=TRUE)
  
  sum_cox_fit <- summary(fit_scoring_MM_landmark)
  scoring_MM_landmark$group = factor(scoring_MM_landmark$group, levels=c(1:3))
  
  # survival curve
  adj_curv <- survfit(Surv(time=death_year, event)~group, data=scoring_MM_landmark, weights = my_weights$weight)
  ...
  ### Survival probability------
  surv10 = Surv_Prov(fit=adj_curv, data=scoring_MM_landmark, time_point=10)
  surv1 = paste0(sprintf("%.1f", surv10[1,3]*100),'%')
  surv2 = paste0(sprintf("%.1f", surv10[2,3]*100),'%')
  surv3 = paste0(sprintf("%.1f", surv10[3,3]*100),'%')
  ...
  dev.new()
  pdf(paste0('250415_Adj_ggsurvplot_landmark',year_num,'yr.pdf'),height=10,width=13)
  print(p)
  dev.off()
  
  # only result table
  dev.new()
  pdf(paste0('250415_Adj_ggsurvplot_landmark',year_num,'yr_onlytable.pdf'),height=5,width=6.5)
  vp <- viewport(x=0.65, y=0.5, width=0.5, height=0.5, just=c("right","top"))
  pushViewport(vp)
...
followup_analysis <- function(year_num, fu_year){
  ...
  dev.new()
  pdf(paste0('250415_Adj_ggsurvplot_followup',fu_year,'yr.pdf'),height=10,width=13)
  print(p)
  dev.off()
  
  # only result table
  dev.new()
  pdf(paste0('250415_Adj_ggsurvplot_followup',fu_year,'yr_onlytable.pdf'),height=5,width=6.5)
  vp <- viewport(x=0.65, y=0.5, width=0.5, height=0.5, just=c("right","top"))
  pushViewport(vp)
  grid.draw(table_grob)
  popViewport()
