...
nrow(mgus_to_mm[mgus_to_mm$mm_outcome==1,])

mgus_to_mm_sub <- mgus_to_mm[mgus_to_mm$mm_outcome==1,] 
nrow(mgus_to_mm_sub)

table(mgus_to_mm_sub$mm_outcome)
...
table(smm_to_mm_sub$mm_outcome)
...
table(denovo_to_mm$event)

table(denovo_to_mm$mm_outcome)

denovo_to_mm_sub <- denovo_to_mm[denovo_to_mm$mm_outcome==1,] 
nrow(denovo_to_mm_sub)

# medication (other) exclude
table(denovo_to_mm_sub$other)
...
# select some columns #
denovo_to_mm_sub2 = denovo_to_mm_sub %>% dplyr::select(event, death_year, SEX_TP_CD, first_mm_age, c(ends_with("_yes")),mp, vd, thali_only, lenal_only, vmp, vtd, other)

denovo_to_mm_sub2 = denovo_to_mm_sub2 %>% 
  bind_rows(smm_to_mm_sub_1yr)
nrow(denovo_to_mm_sub2)
...
names = c("Mgus to symMM", 'smolMM to symMM', "de novo with symMM", "All cohorts")
...
for (i in 1:4){
  ...
  ggsave(paste0("250318_ScoringHist_", labels[i], ".pdf"),height=7,width=10,dpi=300)
...
sprintf("%.1f (%.1f, %.1f)", surv_median(sfit)[1,2]+0.5, surv_median(sfit)[1,3]+0.5, surv_median(sfit)[1,4]+0.5)
sprintf("%.1f (%.1f, %.1f)", surv_median(sfit)[2,2]+0.5, surv_median(sfit)[2,3]+0.5, surv_median(sfit)[2,4]+0.5)
sprintf("%.1f (%.1f, %.1f)", surv_median(sfit)[3,2]+0.5, surv_median(sfit)[3,3]+0.5, surv_median(sfit)[3,4]+0.5)


# median

median1 = sprintf("%.1f (%.1f, %.1f)", surv_median(fit_to_MM_6mths)[1,2]+0.5, surv_median(fit_to_MM_6mths)[1,3]+0.5, surv_median(fit_to_MM_6mths)[1,4]+0.5)
median2 = sprintf("%.1f (%.1f, %.1f)", surv_median(fit_to_MM_6mths)[2,2]+0.5, surv_median(fit_to_MM_6mths)[2,3]+0.5, surv_median(fit_to_MM_6mths)[2,4]+0.5)
median3 = sprintf("%.1f (%.1f, %.1f)", surv_median(fit_to_MM_6mths)[3,2]+0.5, surv_median(fit_to_MM_6mths)[3,3]+0.5, surv_median(fit_to_MM_6mths)[3,4]+0.5)
...
p_val1 = ifelse(    sum_cox_fit$coefficients[1,6]<0.001, "p <.001", paste0(  "p = ", sprintf("%.2f", sum_cox_fit$coefficients[1,6])  )    )

p_val2 = ifelse(    sum_cox_fit$coefficients[2,6]<0.001, "p <.001", paste0(  "p = ", sprintf("%.2f", sum_cox_fit$coefficients[2,6])  )    )
...
################
dev.new()
pdf('250318_ggsurvplot_1yr.pdf',height=10,width=10)
...
# only result table
dev.new()
pdf('250318_ggsurvplot_1yr_onlytable.pdf',height=5,width=5)
...
# median
median1 = sprintf("%.1f (%.1f, %.1f)", surv_median(adj_curv)[1,2]+0.5, surv_median(adj_curv)[1,3]+0.5, surv_median(adj_curv)[1,4]+0.5)

median2 = sprintf("%.1f (%.1f, %.1f)", surv_median(adj_curv)[2,2]+0.5, surv_median(adj_curv)[2,3]+0.5, surv_median(adj_curv)[2,4]+0.5)

median3 = sprintf("%.1f (%.1f, %.1f)", surv_median(adj_curv)[3,2]+0.5, surv_median(adj_curv)[3,3]+0.5, surv_median(adj_curv)[3,4]+0.5)



### Survival probability------
surv10 = Surv_Prov(fit=adj_curv, data=scoring_MM_6mths_filtered, time_point=9.5)
...
##################
dev.new()
pdf('250318_Adj_ggsurvplot_1yr.pdf',height=10,width=13)
...
pdf('250318_Adj_ggsurvplot_1yr_onlytable.pdf',height=5,width=5)
...
dev.new()
pdf("250318_smdplot.pdf",height=7,width=12)
tab_smd_res %>%
  ...
  scale_color_manual(values = col_val, labels = c("MGUS to symMM",'smolMM to symMM', "de novo symMM"), name = "") +
...
mgus_to_mm$status = ifelse(mgus_to_mm$mm_outcome==1, 1,
                           ifelse(mgus_to_mm$death_yn==1, 2, 0))
...
mgus_to_mm <- mgus_to_mm %>% mutate(
  ...
  time_yr = case_when(
    ...
    status == 2 ~ (death_date - first_D472_date)/365.25
...
# pdf("250318_mgus_ci.pdf", height=5, width=6.5)
print(p)
# dev.off()
...
# pdf("250318_mgus_ci_onlytable.pdf", height=5, width=6.5)
print(p)
# dev.off()
...
# 1yr #
final_smm_cohort = smm_to_mm[!(smm_to_mm$hyper_1yr_yes==1 | smm_to_mm$renal_1yr_yes==1 | smm_to_mm$ane_1yr_yes==1 | smm_to_mm$bone_1yr_yes==1),]


nrow(final_smm_cohort) 
...
ci_10y <- sprintf("%.1f", timepoints(ci.smm,times=10)$est[1,1]*100)
ci_half <- timepoints(ci.smm,times=10)$est[1,1]/2

ci_est <- ci.smm$`1 1`$est
ci_time <- ci.smm$`1 1`$time

# median 누적확률보다 같거나 큰 누적확률  의 time value들  중 맨 첫번째 것
ci_half_time <- ci_time[ci_est >= ci_half][1]


# 10 year confidence interval
est = timepoints(ci.smm,times=10)$est[1,1]
se = sqrt(timepoints(ci.smm,times=10)$var[1,1])
...
final_smm_cohort = as.data.frame(final_smm_cohort)


final_smm_cohort <- final_smm_cohort %>% 
  mutate(
    time_yr = ifelse(time_yr == 0, 1e-5, time_yr)
  )

smm_dat = crprep(Tstop = "time_yr", 
                 status = "status",
                 
                 data=final_smm_cohort, 
                 trans=1, 
...
fit_smm = survfit(Surv(Tstart, Tstop, status==1) ~ 1, data= smm_dat, weights=sw)

# font_size=12

p <- ggsurvplot(fit_smm,
                data=smm_dat,
...
p$plot = 
  ...
  p$plot +
  scale_y_continuous(breaks = seq(0, 0.6, 0.1), labels = function(x) x*100, limits=c(0, 0.6)) +
...
pdf("250318_smm_ci.pdf", height=5, width=6.5)
print(p)
dev.off()
...
p <- ggsurvplot(fit_smm, data=smm_dat)
...
pdf("250318_smm_ci_onlytable.pdf", height=5, width=6.5)
print(p)
dev.off()
...
# comparison other terms between cohorts------


# Case 2: sMM to MM ------
smm_to_mm <- read_sas("smm_to_symmm_other.sas7bdat") 
...
# select some columns #
smm_to_mm_sub2 = smm_to_mm_sub %>% dplyr::select(other, mel_thali, mel_lenal)


# Case 3-2: denovo MM------
denovo_to_mm <- read_sas("denovo_symmm_other.sas7bdat") 
...
# select some columns #
denovo_to_mm_sub2 = denovo_to_mm_sub %>% dplyr::select(other, mel_thali, mel_lenal)


# Case 4-1: Concatenate all groups ------
to_MM <- smm_to_mm_sub2 %>%
  
  mutate(group="2:sMM_to_MM") %>% 
  bind_rows(denovo_to_mm_sub2 %>% mutate(group="3:denovoMM"))
...
to_MM$group <- factor(to_MM$group, levels=c('2:sMM_to_MM','3:denovoMM'), labels=c(2:3))


mytable(group ~ ., data=to_MM, method=3, catMethod=0, show.all=T)
